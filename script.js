...
function calendarContributionsMap() {
    // defaults
    var selector = 'body';
    var colorRange = ['#eee', '#f27777', '#dd4242', '#c51919', '#ad0000'];
    var SQUARE_LENGTH = 13;  // square length
...
var now = moment().endOf('day').toDate();
var yearAgo = moment().startOf('day').subtract(1, 'year').toDate();
    var getMapData = d3.time.days(yearAgo, now).map(function (dateElement) {
        return {
            date: dateElement,
            count: (dateElement.getDay() !== 0 && dateElement.getDay() !== 6) ? Math.floor(Math.random() * 60) : Math.floor(Math.random() * 10)
        };
    });
    var map = calendarContributionsMap().data(getMapData).selector('.container').tooltipEnabled(true);
    map();